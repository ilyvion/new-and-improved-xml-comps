...
/// <summary>
/// Harmony patch class for CompPower methods to support improved power grid printing.
/// </summary>
[HarmonyPatch(typeof(CompPower))]
public class CompImprovedPower_CompPrintForPowerGrid
{
    private static readonly FieldInfo ThingDefOf_HiddenConduit = AccessTools.Field(
        typeof(ThingDefOf),
        nameof(ThingDefOf.HiddenConduit)
    );
    private static readonly FieldInfo CompPower_connectParent = AccessTools.Field(
        typeof(CompPower),
        nameof(CompPower.connectParent)
    );
    ...
    private static IEnumerable<CodeInstruction> CompPrintForPowerGrid(
    ...
    private static readonly MethodInfo CompImprovedPower_ShouldPostPrintOnto = AccessTools.Method(
        typeof(CompProperties_ImprovedPower),
    ...
    private static IEnumerable<CodeInstruction> PostPrintOnto(
        IEnumerable<CodeInstruction> instructions,
        ILGenerator generator
    ) =>
        PatchWithShould(
    ...
    )
    {
        ...
        if (is15 || methodName == nameof(CompPower.PostPrintOnto))
        {
            codeMatcher.SearchForward(i =>
                ...
                && f == ThingDefOf_HiddenConduit
            );
            if (!codeMatcher.IsValid)
            {
                Log.Error(
                    $"Could not patch {methodName}, IL does not match expectations: access to ThingDefOf.HiddenConduit not found."
                );
        ...
        {
            codeMatcher.End();
            codeMatcher.SearchBackwards(i =>
                i.opcode == OpCodes.Ldfld
                && i.operand is FieldInfo f
                && f == CompPower_connectParent
            );
            if (!codeMatcher.IsValid)
            {
                Log.Error(
                    $"Could not patch {methodName}, IL does not match expectations: access to CompPower.connectParent not found."
                );
                return codeMatcher.Instructions();
            }
        }
        ...
        if (
            !codeMatcher.IsValid
            || codeMatcher.Opcode != OpCodes.Ldfld
            || codeMatcher.Operand is not FieldInfo f
            || f != CompPower_connectParent
        )
        {
            Log.Error(
