...
internal sealed class GravshipOrbExtension : DefModExtension
{
...
internal sealed class Building_GravEngine_DrawAt_Patch
{
    [HarmonyTranspiler]
...
#pragma warning restore CA1859 // Use concrete types when possible for improved performance
    ...
    )
    {
        ...
        _ = codeMatcher.SearchForward(i =>
            ...
            && ctor.DeclaringType == typeof(Vector3)
        );
        if (!codeMatcher.IsValid)
        {
            Log.Error(
                "Could not patch DrawAt, IL does not match expectations: instantiation of Vector3."
            );
            ...
            .Insert(
                [
                    ...
                    new(
                        OpCodes.Call,
                        AccessTools.Method(
                            typeof(Building_GravEngine_DrawAt_Patch),
                            nameof(UpdateVector3)
                        )
                    ),
                ]
            );
        ...
        _ = codeMatcher.SearchForward(i =>
            i.Calls(
                AccessTools.PropertyGetter(typeof(CachedMaterial), nameof(CachedMaterial.Material))
            )
        );
        if (!codeMatcher.IsValid)
        {
            Log.Error(
                "Could not patch DrawAt, IL does not match expectations: call to CachedMaterial.Material not found."
            );
            return codeMatcher.Instructions();
        }
    ...
    private static void UpdateVector3(Building_GravEngine gravEngine, ref Vector3 vector3)
    {
        ...
        var orbGraphic = extension.orbGraphic;
